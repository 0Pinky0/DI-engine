    ...
    def forward(self, x: torch.Tensor, num_quantiles: Optional[int] = None) -> Dict:
        ...
        if len(x.shape) == 3:
            ...
            for i in range(x.shape[1]):
                _x = x[:, i, :]
                _x = _x.repeat(num_quantiles, 1)
                
                q_x = _x * q_quantile_net  # 4*32,64
                logit_x = _x * logit_quantile_net

                q = self.Q(q_x).reshape(num_quantiles, batch_size, -1)
                logit = self.Q(logit_x).reshape(num_quantiles, batch_size, -1).mean(0)
                # print(logit.size())
                _logit = logit.squeeze(0)
                q_list.append(q)
                logit_list.append(_logit)
            # print("logit_list",logit_list)
            q_combined = torch.stack(q_list)
            logit_combined = torch.stack(logit_list).unsqueeze(0)
            
                
            return {'logit': logit_combined, 'q': q_combined, 'quantiles': q_quantiles}
        elif len(x.shape) == 2:
...
class FQFHead(nn.Module):
    ...
    def quantile_net(self, quantiles: torch.Tensor) -> torch.Tensor:
        ...
        # quantile_net = torch.cos(self.sigma_pi.to(quantiles) * quantiles.view(batch_size,  5, -1))
        # quantile_net = quantiles.view(1,  5, -1)
        quantile_net = self.fqf_fc(quantiles)  # (batch_size, num_quantiles, hidden_size)
        return quantile_net
    ...
    def forward(self, x: torch.Tensor, num_quantiles: Optional[int] = None) -> Dict:
        ...
        # assert entropies.shape == (batch_size, 5, 1)
        ...
        tau_0 = torch.zeros(q_quantiles.size()).to(x)
        if tau_0.dim() == 3 and q_quantiles.dim() == 3:
            q_quantiles = torch.cat((tau_0, q_quantiles), dim=2)
        else:
        ...
        q_quantiles_hats = (q_quantiles[:, :] + q_quantiles[:, :]).detach() / 2.  # (batch_size, num_quantiles)

        # NOTE(rjy): reparameterize q_quantiles_hats
        q_quantile_net = self.quantile_net(q_quantiles)  # [batch_size, num_quantiles, hidden_size(64)]
        # x.view[batch_size, 1, hidden_size(64)]
        q_x = (x.view(q_quantile_net.size()) * q_quantile_net)  # [batch_size, num_quantiles, hidden_size(64)]
        ...
        with torch.no_grad():
            ...
            q_tau_i_x = (x.view(q_tau_i_net.size()) * q_tau_i_net)   # [batch_size, (num_quantiles-1), hidden_size(64)]
