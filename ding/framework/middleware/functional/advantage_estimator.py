...
def gae_estimator(cfg: EasyDict, policy: Policy, buffer_: Optional[Buffer] = None) -> Callable:
    ...
    def _gae(ctx: "OnlineRLContext"):
        ...
        # trajectories = [x for x in trajectories if x['action'] != -1]
        ...
        if buffer_ is None:
            ...
            if data[0]['obs']['agent_state'].shape == obs_shape:
                pass
            elif data[0]['obs']['agent_state'].shape[0] == 1 and data[0]['obs']['agent_state'].shape[1:] == obs_shape:
                for d in data:
                    d['obs']['agent_state'] = d['obs']['agent_state'].squeeze(0)
                    d['next_obs']['agent_state'] = d['next_obs']['agent_state'].squeeze(0)
                if 'logit' in data[0]:
                    for d in data:
