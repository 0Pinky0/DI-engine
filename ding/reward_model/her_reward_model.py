...
class HerRewardModel:
    ...
    def estimate(
        ...
        for idx in indices:
            # print(episode[idx]['obs'])
            obs, _, _ = split_func(episode[idx]['obs']["agent_state"])
            next_obs, _, achieved_goal = split_func(episode[idx]['next_obs']["agent_state"])
            for k in range(self._her_replay_k):
                ...
                _, _, new_desired_goal = split_func(episode[idx]['next_obs']["agent_state"])
                timestep = {
                    k: copy.deepcopy(v)
                    for k, v in enumerate(episode) if k not in ['obs', 'next_obs', 'reward']
                }
                # print(obs)
