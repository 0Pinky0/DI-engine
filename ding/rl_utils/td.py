...
def dist_nstep_td_error(
    ...
    for i in range(1, nstep):
        reward_factor[i] = gamma * reward_factor[i - 1]
        # reward_factor[i] = reward_factor[i].float()
    ...
    if value_gamma is None:
        ...
        # value_gamma = value_gamma.repeat_interleave(repeats=(reward.shape[0] // value_gamma.shape[0]), dim=0)
        # print(value_gamma.size(), reward.size(), support.size())
        target_z = reward + (1 - done) * value_gamma * support
    target_z = target_z.clamp(min=v_min, max=v_max)
    b = (target_z - v_min) / delta_z
    l = b.floor().long()
...
def qrdqn_nstep_td_error(
    ...
    # print(batch_range.shape, action.shape)
...
def iqn_nstep_td_error(
    ...
    reward_factor = torch.ones(nstep).to(device).to(dtype=torch.float)
    for i in range(1, nstep):
        reward_factor[i] = gamma * reward_factor[i - 1]
    reward = torch.matmul(reward_factor, reward.to(dtype=torch.float))
    if value_gamma is None:
