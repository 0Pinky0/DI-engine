...
def default_preprocess_learn(
    ...
    if isinstance(elem['action'], (np.ndarray, torch.Tensor)) and elem['action'].dtype in [np.int64, torch.int64]:
        ...
        data['obs']['agent_state'] = data['obs']['agent_state'].to(dtype=torch.float32)
        if 'next_obs' in data:
            data['next_obs']['agent_state'] = data['next_obs']['agent_state'].to(dtype=torch.float32)
    else:
    ...
    if 'value' in data and data['value'].dim() == 2 and data['value'].shape[1] == 1:
        data['value'] = data['value'].squeeze(-1).to(dtype=torch.float32)
    if 'adv' in data and data['adv'].dim() == 2 and data['adv'].shape[1] == 1:
        data['adv'] = data['adv'].squeeze(-1).to(dtype=torch.float32)
    ...
    if data['done'].dim() == 2 and data['done'].shape[1] == 1:
        data['done'] = data['done'].squeeze(-1).float()
    ...
    if use_nstep:
        ...
        if len(reward.shape) == 1:
            reward = reward.unsqueeze(1).to(dtype=torch.float32)
        # reward: (batch_size, nstep) -> (nstep, batch_size)
        data['reward'] = reward.permute(1, 0).contiguous().float()
    else:
        if data['reward'].dim() == 2 and data['reward'].shape[1] == 1:
            data['reward'] = data['reward'].squeeze(-1).to(dtype=torch.float32)
