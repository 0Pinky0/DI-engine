...
class PPOPolicy(Policy):
    ...
    def _forward_learn(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        """
        if os.path.exists(self._cfg.load_path):
            self.saved_model = torch.load(self._cfg.load_path)
        ...
        for epoch in range(self._cfg.learn.epoch_per_collect):
            ...
            for batch in split_data_generator(data, self._cfg.learn.batch_size, shuffle=False):
                for name, param in self._learn_model.named_parameters():
                    if torch.isnan(param).any():
                        print('reload model')
                        self._learn_model.load_state_dict(self.saved_model, strict=True)
                output = self._learn_model.forward(batch['obs'], mode='compute_actor_critic')
                if torch.isnan(output['logit']).any():
                    print(batch['obs'], output['logit'])
                ...
                total_loss = 2*ppo_loss.policy_loss + wv * ppo_loss.value_loss - we * ppo_loss.entropy_loss
    ...
    def _forward_eval(self, data: Dict[int, Any]) -> Dict[int, Any]:
        ...
        if self._cuda:
            output = to_device(output, 'cpu')
        transition = default_decollate(output)
        return {i: d for i, d in zip(data_id, transition)}
...
class PPOSTDIMPolicy(PPOPolicy):
    ...
    def _model_encode(self, data):
        ...
        if isinstance(x, dict):
            if 'agent_state' in x:
                x = x['agent_state']
                x = self._model.encoder(x)
                y = y['agent_state']
                y = self._model.encoder(y)
