...
class ACERPolicy(Policy):
    ...
    def _init_learn(self) -> None:
        ...
        self._obs_shape = self._cfg.model.obs_shape
    ...
    def _data_preprocess_learn(self, data: List[Dict[str, Any]]):
        ...
        tensor_obs = {}
        tensor_obs['action_mask'] = data['obs']['action_mask']
        # tensor_obs['agent_state'] = torch.cat((data['obs']['agent_state'], data['next_obs']['agent_state'][-1:]), dim=0).reshape(-1, self._obs_shape)
        tensor_obs['agent_state'] = data['obs']['agent_state'].reshape(-1, self._obs_shape)
        data['obs_plus_1'] = ttorch.as_tensor(tensor_obs)
        data['logit'] = data['logit'].reshape(self._unroll_len, -1, self._action_shape)  # shape T,B,env_action_shape
        data['action'] = data['action'].reshape(self._unroll_len, -1)  # shape T,B,
        ...
        data['weight'] = data['weight'].reshape(self._unroll_len, -1) if data['weight'] else None  # shape T,B
        return data
    ...
    def _forward_learn(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        action_data = self._learn_model.forward(data['obs_plus_1']['agent_state'], mode='compute_actor')
        q_value_data = self._learn_model.forward(data['obs_plus_1']['agent_state'], mode='compute_critic')
        avg_action_data = self._target_model.forward(data['obs_plus_1']['agent_state'], mode='compute_actor')
    ...
    def _reshape_data(
        ...
        values = q_value_data['value'].reshape(
            self._unroll_len, -1, self._action_shape
        )  # shape (T+1),B,env_action_shape
